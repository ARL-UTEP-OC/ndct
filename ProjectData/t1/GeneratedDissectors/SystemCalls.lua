...
function SystemCalls_proto.dissector(buffer,pinfo,tree)
    ...
    if pinfo.abs_ts >= 1596036423.0 and pinfo.abs_ts <= 1596036425.0 then
       ...
       subtree:add(tostring("Original Timestamp: "),tostring("2020-07-29T15:27:03"))
    ...
    if pinfo.abs_ts >= 1596036424.0 and pinfo.abs_ts <= 1596036426.0 then
       local subtree = tree:add(SystemCalls_proto,"SystemCalls Log")
       ...
       subtree:add(tostring("Original Timestamp: "),tostring("2020-07-29T15:27:04"))
       subtree:add(tostring("Log Data: "), mycomplientstr)
    end
    if pinfo.abs_ts >= 1596036424.0 and pinfo.abs_ts <= 1596036426.0 then
       local subtree = tree:add(SystemCalls_proto,"SystemCalls Log")
    ...
    if pinfo.abs_ts >= 1596036424.0 and pinfo.abs_ts <= 1596036426.0 then
       local subtree = tree:add(SystemCalls_proto,"SystemCalls Log")
       local mycomplientstr = tostring("cat /home/kali/eceld-netsys/eceld/plugins/collectors/auditd/raw/1596036421_auditd.txt")
